...
Core.prototype._redraw = function() {
  const _this = this;
  _this.redrawCount++;
  const { dom, body, options, props }  = this;
  ...
  fastdom.measure(function () {
    const domRoot = dom.root;
    const { clientHeight, offsetHeight, offsetWidth, clientWidth } = domRoot;
    const centerContainerClientHeight =  dom.centerContainer.clientHeight;

    DateUtil.updateHiddenDates(options.moment, body, options.hiddenDates);
    ...
    var autoHeight = props.top.height + contentHeight + props.bottom.height +
      props.borderRootHeight + borderProps.top + borderProps.bottom;
    ...
    // resize the panels
    fastdom.mutate(function () {
      const backgroundHeight = `${props.background.height}px`;
      const centerContainerHeight = `${props.centerContainer.height}px`;
      dom.background.style.setProperty('height', backgroundHeight);
      dom.backgroundVertical.style.setProperty('height', backgroundHeight);
      dom.backgroundHorizontal.style.setProperty('height', centerContainerHeight);
      dom.centerContainer.style.setProperty('height', centerContainerHeight);
      dom.leftContainer.style.setProperty('height', `${props.leftContainer.height}px`);
      dom.background.style.setProperty('width', `${props.background.width}px`);
      dom.backgroundVertical.style.setProperty('width', `${props.centerContainer.width}px`);
      dom.backgroundHorizontal.style.setProperty('width', `${props.background.width}px`);
      dom.centerContainer.style.setProperty('width', `${props.center.width}px`);
      dom.bottom.style.setProperty('width', `${props.bottom.width}px`);

      dom.background.style.setProperty('height', '0');
      dom.background.style.setProperty('height', '0');
      dom.backgroundVertical.style.setProperty('left', `${(props.left.width + borderProps.left - 2)}px`);
      dom.backgroundVertical.style.setProperty('top', '0');
      dom.backgroundHorizontal.style.setProperty('left', '0');
      dom.backgroundHorizontal.style.setProperty('top', `${props.top.height}px`);
      dom.centerContainer.style.setProperty('left', `${props.left.width}px`);
      dom.centerContainer.style.setProperty('top', `${props.top.height}px`);
      dom.leftContainer.style.setProperty('left', '0');
      dom.leftContainer.style.setProperty('top', `${props.top.height}px`);
      dom.bottom.style.setProperty('left', `${props.left.width}px`);
      dom.bottom.style.setProperty('top', `${(props.top.height + props.centerContainer.height)}px`);
    });
    
    // update the scrollTop, feasible range for the offset can be changed
    // when the height of the Core or of the contents of the center changed
    _this._updateScrollTop();

    // reposition the scrollable contents
    fastdom.mutate(function () {
      let offset = props.scrollTop;
      if (options.orientation.item != 'top') {
        offset += Math.max(props.centerContainer.height - props.center.height -
          borderProps.top - borderProps.bottom, 0);
      }
      
      dom.center.style.setProperty('transform', `translate(0px, ${offset}px`);
      dom.left.style.setProperty('transform', `translate(0px, ${offset}`);
      dom.right.style.setProperty('transform', `translate(0px, ${offset}px`);
    });
    ...
    if (!body.reduceRedraw) { // if reduceRedraw true, then dont execute this block
      var MAX_REDRAW = 5;
      if (resized) {
        if (_this.redrawCount < MAX_REDRAW) {
          body.emitter.emit('_change');
          return;
        }
      } else {
        _this.redrawCount = 0;
    ...
    _this.initialDrawDone = true;

    //Emit public 'changed' event for UI updates, see issue #1592
    body.emitter.emit("changed");
  });
};
...
Core.prototype._startAutoResize = function () {
  ...
  this._onResize = function() {
    ...
    if (meDom && meDomRoot) {
      const rootOffsetHeight = meDomRoot.offsetHeight;
      const rootOffsetWidth = meDomRoot.offsetWidth;
      const meProps = me.props;
  ...
  if (me.dom && me.dom.root) {
      const { offsetWidth, offsetHeight } = me.dom.root;
