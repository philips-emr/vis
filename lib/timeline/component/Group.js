...
Group.prototype._updateVisibleItems = function(orderedItems, oldVisibleItems, range) {
  ...
  for (i = 0; i < visibleItems.length; i++) {
    item = visibleItems[i];
    var filterFit = false;
    if (range.options.itemsFit) {
      range.options.itemsFit.forEach((itens) => {
        if (new Date(itens.time).getTime() == item.data.start.getTime()) filterFit = true;
      });
    }
    if (range.options.gap == 0 && !filterFit) {
      item.hide();
    } else {
