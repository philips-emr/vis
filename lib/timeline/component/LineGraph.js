  ...
  if (options) {
    ...
    if (options.graphHeight === undefined && options.height !== undefined) {
      ...
      this.updateSVGheightOnResize = true;
        ...
        && parseInt(options.graphHeight.toString().replace('px', '')) < this.body.domProps.centerContainer.height
    ) {
      this.updateSVGheight = true;
    }
...
LineGraph.prototype.redraw = function () {
  ...
  if (this.updateSVGheight === true) {
    const propsHeight = `${this.props.height}px`;
...
LineGraph.prototype._toggleAxisVisiblity = function (axisUsed, axis) {
  let changed = false;
  if (axisUsed === false) {
    if (axis.dom.frame && axis.dom.frame.parentNode && axis.hidden === false) {
      axis.hide();
      changed = true;
    }
  }
...
LineGraph.prototype._convertXcoordinates = function (datapoints) {
  const dataRegion = document.querySelector('.data-region-timeline');
  const widthTimeline = dataRegion ? dataRegion.offsetWidth : 0;
  ...
  for (let i = 0; i < datapoints.length; i++) {
    ...
    if (this.body.range.options && (this.body.range.options.gap === 0 || (this.body.range.options.itemsFit && this.body.range.options.itemsFit.some(x => x.type == GAPCHARTNOW)))) {
      ...
      let xToPercent = (dataPointItem.x.getTime() - this.body.range.start) * 100 / factor;
      ...
      if (this.body.totalizer && this.body.totalizer.periods && this.body.totalizer.periods.length > 0) {
        ...
        var filteredTotalizers = totalizers.filter(x => { return x.end.valueOf() < dataPointItem.x.getTime() });
        var columnWidth = this.props.width / (factor / (3600000 * this.body.range.options.gap) + totalizers.length);
        dataPointItem.screen_x = ((this.props.width - totalizers.length * columnWidth) * xToPercent / 100) + filteredTotalizers.length * columnWidth;
      } else {
        dataPointItem.screen_x = this.props.width * xToPercent / 100;
      }
    }
