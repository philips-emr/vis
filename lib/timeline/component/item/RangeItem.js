...
RangeItem.prototype.repositionX = function(limitSize, group, visibleItemIndex) {
  ...
  const elementOffSetWidth = group.body.domProps.center.width;
  const widthElement = parseFloat(elementOffSetWidth / elementHeaderWidthItem.length).toFixed(2);
  ...
  const isTableMode = ['tablemode', 'tablemode_multiple_values'].indexOf(prop.type) > -1;
  ...
  if (group.options.gap == 0 && !isTableMode) {
    ...
    Object.values(elementHeaderWidth.children).forEach((item) => {
      ...
      if (dateElement > _currentDateItem) index++;
      if (dateElementEnd > _currentDateItem) indexEnd++;
    });
  ...
  // Take element width 'tl-setting-bar__item' of the handler timeline and calculate width
    ...
    if (isTableMode) {
      const indexForColumnPositionCalculation = group.options.gap === 0 && visibleItemIndex ? visibleItemIndex : dataIdItemSplit[1];

      const calcPositionStart = (parseInt(indexForColumnPositionCalculation) * widthElement);
      const calcPositionEnd = ((parseInt(indexForColumnPositionCalculation) + 1) * widthElement);
  ...
  const boxWidth = Math.max(end - start, 1) + 2;
  if (this.overflow) {
    this.left = start;
