...
RangeItem.prototype.repositionX = function(limitSize, group) {
  ...
  if (group.options.gap == 0) {
    ...
    elementHeaderWidthItem.forEach((item) => {
  ...
  // Take element width 'tl-setting-bar__item' of the handler timeline and calculate width
    ...
    if (prop.type == 'tablemode' || prop.type == 'tablemode_multiple_values') {
      ...
      const dataIdItemSplit_1 = parseInt(dataIdItemSplit[1]);
      this.dom.box.classList.remove('tablemode-fit');
      const calcPositionStart = (dataIdItemSplit_1 * widthElement);
      const calcPositionEnd = ((dataIdItemSplit_1 + 1) * widthElement);
  ...
  if (limitSize === undefined || limitSize === true) {
    ...
    const parentWidthPer2 = 2 * parentWidth;
