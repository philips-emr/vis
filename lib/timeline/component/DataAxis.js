...
DataAxis.prototype._redrawLabels = function () {
  ...
  lines.forEach(
    line=> {
      ...
      let labelClass = 'vis-y-axis';
      let lineClass = 'vis-grid vis-horizontal';
      if (isMiddle) {
      ...
      if (this.options['showMinorLabels'] && isMajor === false) {
        ...
        this._redrawLabel(y - 2, line.val, orientation, labelClass, this.props.minorCharHeight);
      }
      if (isMajor) {
        if (y >= 0) {
          labelClass += ' vis-major'
          this._redrawLabel(y - 2, line.val, orientation, labelClass, this.props.majorCharHeight);
        }
      }

      if (this.master === true) {
        const linesOffset = this.options.linesOffsetY;
        if (isMajor) {
          lineClass += ' vis-major';
          this._redrawLine(y + linesOffset, orientation, lineClass, this.options.majorLinesOffset, this.props.majorLineWidth);
        } else if (this.options['showMinorLines']) {
          lineClass += ' vis-minor';
          this._redrawLine(y + linesOffset, orientation, lineClass, this.options.minorLinesOffset, this.props.minorLineWidth);
        }
      }
    });
