...
ItemSet.prototype.redraw = function(timeline) {
  ...
  const widthContainerVIS = range.body.domProps.center.width;
  const elementHeaderWidthItem = document.querySelectorAll('.tl-setting-bar .tl-setting-bar__item');
  const widthElement = parseFloat(widthContainerVIS / elementHeaderWidthItem.length).toFixed(2);
...
ItemSet.prototype._onUpdate = function(ids) {
  var me = this;
  const widthContainerVIS = this.body.range.body.domProps.center.width;
  const elementHeaderWidthItem = document.querySelectorAll('.tl-setting-bar .tl-setting-bar__item');
  const widthElement = parseFloat(widthContainerVIS / elementHeaderWidthItem.length).toFixed(2);
...
ItemSet.prototype._onRemoveGroups = function(ids) {
  ...
  this.body && this.body.emitter.emit('_change', {queue: true});
};
...
ItemSet.prototype._onDragStartAddItem = function (event) {
  ...
  const x = event.center.x - frameRect.left - 10; // minus 10 to compensate for the drag starting as soon as you've moved 10px
...
ItemSet.prototype._onDrag = function (event) {
  if (this.touchParams.itemProps) {
    ...
    const xOffset = domRootOffsetLeft + this.body.domProps.left.width;
    var scale = this.body.util.getScale();
    var step = this.body.util.getStep();
