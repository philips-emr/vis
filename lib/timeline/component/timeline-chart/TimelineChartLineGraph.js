...
class TimelineChartLineGraph extends LineGraph {
      ...
      if (groupIds.length > 0) {
        ...
        for (i = 0; i < groupIds.length; i++) {
          ...
          if (group.group.type == 'infusionrate' || group.group.type == 'bolus') {
            InfusionRate.drawBackground(group, this.framework, this.body);
          }
  ...
  _convertAvgYcoordinates(datapoints, group, actualY, previousY) {
    ...
    for (var i = 0; i < datapoints.length; i++) {
      const dataPonintI = datapoints[i];
      if (dataPonintI.referenceLine) {
        let convertedValue = 0;
        const maxValue = dataPonintI.y;
        const minValue = dataPonintI.y;
        ...
        const maxValue = dataPonintI.maxValue;
        const minValue = dataPonintI.minValue;
        const avgValue = dataPonintI.avgValue;
        ...
        if(avgValue){
          ...
          dataPonintI.screen_yMin = baseScreenY * ((minValue - range.min) * 100 / graphScale) / 100;
          dataPonintI.screen_yMax = baseScreenY * ((maxValue - range.min) * 100 / graphScale) / 100;
