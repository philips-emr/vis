...
InfusionRate.createSVGElement = function (subGroup, zeroPosition, style, yMaximum, yMinimum, framework, body) {
    ...
    let screen_x = subGroup[0].screen_x || subGroup[0].x;
    let screen_y = subGroup[0].screen_y || subGroup[0].y;
    let screen_x_end = subGroup[length - 1].screen_x || subGroup[length - 1].x;
    if (!(isNaN(screen_x) && isNaN(zeroPosition - screen_y) && isNaN(screen_x_end))) {
      ...
      rectElement.setAttribute('width', screen_x_end - screen_x);
      rectElement.setAttribute('height', zeroPosition - screen_y);
