...
GraphGroup.prototype.getYRange = function (groupData, groupId, group) {
  ...
  if (
    (group.options.drawPoints && group.options.drawPoints.style === 'arrow-avg') ||
    (group.style && group.style.drawPoints && group.style.drawPoints.style === 'arrow-avg')
    ) {
    const values = [];
    groupData.map(d => values.push(d.data.max, d.data.min));
