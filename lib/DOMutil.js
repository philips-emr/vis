...
exports.drawPoint = function(x, y, groupTemplate, JSONcontainer, svgContainer, labelObj) {
  ...
  switch (groupTemplate.style) {
    ...
    case 'arrow-avg':
      ...
      const yOffsetTop = 30; // This bring the top arrow, to top.
      const yOffsetBottom = 30; // This push the bottom arrow to down
      const lineHeight = 10;

      const tLeftPoint = `${v.x - xOffset},${v.y - yOffsetTop}`;
      const tRightPoint = `${v.x + groupTemplate.size + xOffset},${v.y - yOffsetTop}`;
      const tBottomPoint = `${v.x + groupTemplate.size * 0.5},${v.y - (2 + yOffsetTop) + groupTemplate.size}`;
      const tLinePoint = `${v.x + groupTemplate.size * 0.5},${v.y - (2 + yOffsetTop) + lineHeight + groupTemplate.size}`;

      const bLeftPoint = `${v.x - xOffset},${v.y + yOffsetBottom + groupTemplate.size}`;
      const bRightPoint = `${v.x + groupTemplate.size + xOffset},${v.y + yOffsetBottom + groupTemplate.size}`;
      const bTopPoint = `${v.x + groupTemplate.size * 0.5},${v.y + (2 + yOffsetBottom)}`;
      const bLinePoint = `${v.x + groupTemplate.size * 0.5},${v.y + (2 + yOffsetBottom) - lineHeight}`;
